...
export interface OwnerLoadedRoute {
  resource: "owner";
  params: {
...
export async function loadOwnerRoute({
  baseUrl,
}: OwnerRoute): Promise<OwnerLoadedRoute | NotFoundRoute | ErrorRoute> {
  if (!baseUrl) {
    baseUrl = determineSeed();
  }

  const parsedDid = utils.parseNodeId(config.owner.did);
  if (!parsedDid) {
    return {
      resource: "error",
      params: {
...
export function ownerRouteToPath(route: OwnerRoute): string {
  return [nodePath(route.baseUrl), "owner", route.did].join("/");
}

export function ownerTitle(route: OwnerLoadedRoute): string[] {
